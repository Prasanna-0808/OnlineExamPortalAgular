...
//         this.markEnrolledPackages();
//         this.filterEnrollablePackages();
//       },
//       error: (err) => console.error('User packages load failed', err)
//     });
//   }

//   markEnrolledPackages(): void {
//     const enrolledIds = this.userPackages.map(pkg => pkg.id);
//     this.packages = this.packages.map(pkg => ({
//       ...pkg,
//       isEnrolled: enrolledIds.includes(pkg.id)
//     }));
//   }

  
// filterEnrollablePackages(): void {
//   const enrolledIds = this.userPackages.map(pkg => pkg.id);
//   this.enrollablePackages = this.packages.filter(pkg => !enrolledIds.includes(pkg.id));
// }


//   requestPackage(pkgId: number): void {
//     alert(`Request raised for package ID: ${pkgId}`);
//     // You can later implement actual request logic here
//   }
// }
...
export class PackagesComponent implements OnInit {
  ...
  //     next: (res) =>{console.log('Assessments:', res);  this.assessmentsInPackage = res},
  //     error: (err) => console.error('Failed to load assessments', err)
  //   });
  // }

  // requestAssessment(assessmentId: number): void {
  //   alert(`Request raised for assessment ID: ${assessmentId}`);
  //   // Future: Send request to InstructorRequestView or CandidateRequest API
  // }
